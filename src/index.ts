...
export function shouldSendConfiguredOwner(ownerId: string, ownerIdMode: OwnerContextMode): boolean {
  return ownerIdMode === "configured" && Boolean(ownerId) && ownerId !== "default";
}

export function withConfiguredOwner<T extends Record<string, unknown>>(
  body: T,
  ownerId: string,
  ownerIdMode: OwnerContextMode,
): T & { owner_id?: string } {
  if (!shouldSendConfiguredOwner(ownerId, ownerIdMode)) return body;
  return { ...body, owner_id: ownerId };
}

function addOwnerParam(params: URLSearchParams, ownerId: string, ownerIdMode: OwnerContextMode): void {
  ...
  if (shouldSendConfiguredOwner(ownerId, ownerIdMode)) params.set("owner_id", ownerId);
}
...
export function formatCortexToolResult(label: string, result: CortexToolResult | null): string {
  if (!result) {
    return `${label} failed: Cortex returned no result.`;
  }
  return `${label}:\n${JSON.stringify(result, null, 2)}`;
...
function isApprovalGatedRecord(record: Record<string, unknown>): boolean {
  return record.requires_approval === true;
}

function collectApprovalGatedItems(
  value: unknown,
  path = "$",
...
function buildCompanyBrainActionSafety(payload: CompanyBrainContextPayload): Record<string, unknown> {
...
export function formatCompanyBrainContext(
  payload: CompanyBrainContextPayload,
...
function parseConfig(raw: unknown): EvaMemoryConfig {
  ...
  const parsedCompanyBrainContextSourceScope: CompanyBrainSourceScope =
    c.companyBrainContextSourceScope === "internal" || c.companyBrainContextSourceScope === "customer_accounts"
...
class CortexClient {
  ...
  constructor(
    ...
    private ownerIdMode: OwnerContextMode,
  ...
  private ownerBody<T extends Record<string, unknown>>(body: T): T & { owner_id?: string } {
    return withConfiguredOwner(body, this.ownerId, this.ownerIdMode);
  }

  private ownerParams(initial: Record<string, string> = {}): URLSearchParams {
  ...
  async retrieve(query: string, tokenBudget: number, mode = "auto"): Promise<RetrievalResult | null> {
    ...
    return this.post<RetrievalResult>(
  ...
  remember(conversation: Array<{ role: string; content: string }>, sessionId?: string, shadow = false): Promise<unknown> {
    ...
    return this.post(
      path,
      this.ownerBody({ conversation, session_id: sessionId, source_session_id: sessionId }),
      30000,
    );
  }
  ...
  async search(query: string, limit = 10) {
    ...
    return this.post<{ items: Record<string, unknown>[]; total: number }>(
      ...
      this.ownerBody({ query, top_k: limit }),
    );
  }
  ...
  async forget(memoryId: string) {
    ...
    return this.del<{ memory_id: string; deleted: boolean }>(
      `/api/v1/memories/${encodeURIComponent(memoryId)}${query ? `?${query}` : ""}`,
    );
  }
  ...
  async listContradictions() {
    ...
    return this.get<{ items: Record<string, unknown>[]; total: number; errors: string[] }>(
      this.withQuery("/api/v1/contradictions", params),
    );
  }

  async resolveContradiction(id: string, resolution: string) {
    return this.patch<Record<string, unknown>>(
      `/api/v1/contradictions/${encodeURIComponent(id)}`,
      this.ownerBody({ resolution }),
    );
  }

  // --- Commitments ---

  async addCommitment(description: string, dueAt?: string) {
    return this.post<Record<string, unknown>>(
      "/api/v1/commitments",
      this.ownerBody({ content: description, due_at: dueAt }),
    );
  }

  async updateCommitment(commitmentId: string, commitmentStatus: string) {
    return this.patch<Record<string, unknown>>(
      `/api/v1/commitments/${encodeURIComponent(commitmentId)}`,
      this.ownerBody({ status: commitmentStatus }),
    );
  }

  async listCommitments(status?: string) {
    const params = this.ownerParams();
    if (status) params.set("status", status);
    return this.get<{ commitments?: Record<string, unknown>[]; total?: number }>(
      this.withQuery("/api/v1/commitments", params),
    );
  }
  ...
  async listInsights(status = "pending", limit = 5) {
    ...
    return this.get<{ insights?: Record<string, unknown>[]; count?: number }>(
      this.withQuery("/api/v1/insights", params),
    );
  }

  // --- Open Loops ---

  async addOpenLoop(description: string) {
    return this.post<Record<string, unknown>>(
      "/api/v1/open-loops",
      this.ownerBody({ content: description }),
    );
  }

  async resolveOpenLoop(loopId: string) {
    return this.patch<Record<string, unknown>>(
  ...
  async listOpenLoops(status?: string) {
    const params = this.ownerParams();
    if (status) params.set("status", status);
    return this.get<{ open_loops?: Record<string, unknown>[]; total?: number }>(
      this.withQuery("/api/v1/open-loops", params),
    );
  }
  ...
  async listEntities(options: {
    ...
    return this.get<CortexToolResult>(
      this.withQuery("/api/v1/entities", params),
    );
  }
  ...
  async getEntityDetail(entityId: string) {
    const params = this.ownerParams();
    return this.get<CortexToolResult>(
      this.withQuery(`/api/v1/entities/${encodeURIComponent(entityId)}`, params),
    );
  }
  ...
  async queryGraph(options: {
    ...
    return this.get<CortexToolResult>(
      this.withQuery("/api/v1/graph", params),
    );
  }
  ...
  async getCornerstones(): Promise<Array<{ label: string; content: string }> | null> {
    ...
    return this.get<Array<{ label: string; content: string }>>(
      this.withQuery("/api/v1/cornerstones", params),
      3000,
    );
  }
  ...
  async listMemories(perPage = 200, page = 1, includeEmbeddings = false): Promise<{ items: any[]; total: number; pages: number } | null> {
    ...
    return this.get<{ items: any[]; total: number; pages: number }>(
      this.withQuery("/api/v1/memories", params),
...
function shouldAttemptCompanyBrainContext(prompt: string | undefined, cfg: EvaMemoryConfig): boolean {
  if (cfg.companyBrainContextMode !== "auto") return false;
  if (cfg.companyBrainContextAccountId || cfg.companyBrainContextAccountKey || cfg.companyBrainContextSearch) return true;
  return COMPANY_BRAIN_CONTEXT_KEYWORDS.test(prompt ?? "");
}

function normalizeCompanyBrainAccounts(result: CompanyBrainToolResult | null): Record<string, unknown>[] {
  if (!result) return [];
  const accounts = result.accounts ?? result.items ?? result.results;
  return Array.isArray(accounts)
    ? accounts.map(asRecord).filter((account): account is Record<string, unknown> => Boolean(account))
    : [];
}

function companyBrainAccountId(account: Record<string, unknown>): string {
  return firstString(account, ["id", "account_id", "customer_id"]);
}

function companyBrainAccountKey(account: Record<string, unknown>): string {
...
const cortexPlugin = {
  ...
  register(api: OpenClawPluginApi) {
    ...
    api.logger.info(
      `cortex: registered (cortex=${cfg.cortexUrl}, ownerMode=${cfg.ownerIdMode}, owner=${cfg.ownerId}, recall=${cfg.autoRecall}, capture=${cfg.autoCapture}, shadow=${cfg.shadowMode})`,
    );
    ...
    api.registerTool(
      {
        ...
        parameters: Type.Object({}),
        async execute(_toolCallId: string, _params: unknown): Promise<any> {
          try {
    ...
    api.registerTool(
      {
        ...
        async execute(_toolCallId: string, params: unknown): Promise<any> {
          const { description, due_at } = params as { description: string; due_at?: string };
          try {
    ...
    api.registerTool(
      {
        ...
        parameters: Type.Object({
          entity_type: Type.Optional(Type.String({ description: "Optional entity type filter, such as person, organization, project, or topic" })),
          search: Type.Optional(Type.String({ description: "Optional entity search text" })),
          limit: Type.Optional(Type.Number({ description: "Max entities to return, 1-200 (default: 50)" })),
          offset: Type.Optional(Type.Number({ description: "Zero-based entity offset (default: 0)" })),
        }),
        async execute(_toolCallId: string, params: unknown): Promise<any> {
          const { entity_type, search, limit, offset } = params as {
            entity_type?: string;
            search?: string;
            limit?: number;
            offset?: number;
          ...
          try {
            ...
            return { content: [{ type: "text" as const, text: formatCortexToolResult("Cortex entities", result) }] };
          } catch (err) {
            return { content: [{ type: "text" as const, text: `Cortex entities list failed: ${String(err)}` }] };
          }
        },
      },
      { name: "cortex_entities_list" },
    ...
    api.registerTool(
      {
        ...
        parameters: Type.Object({
          entity_id: Type.String({ description: "Stable Cortex entity ID from cortex_entities_list" }),
        }),
        async execute(_toolCallId: string, params: unknown): Promise<any> {
          const { entity_id } = params as { entity_id: string };
          try {
            const result = await client.getEntityDetail(entity_id);
            return { content: [{ type: "text" as const, text: formatCortexToolResult("Cortex entity detail", result) }] };
          } catch (err) {
            return { content: [{ type: "text" as const, text: `Cortex entity detail failed: ${String(err)}` }] };
          }
        },
      },
      { name: "cortex_entity_detail" },
    ...
    api.registerTool(
      {
        ...
        parameters: Type.Object({
          center_entity_id: Type.Optional(Type.String({ description: "Optional entity ID to center the graph on" })),
          max_depth: Type.Optional(Type.Number({ description: "Max graph traversal depth, 1-5 (default: 2)" })),
          max_nodes: Type.Optional(Type.Number({ description: "Max graph nodes to return, 1-200 (default: 50)" })),
        }),
        async execute(_toolCallId: string, params: unknown): Promise<any> {
          const { center_entity_id, max_depth, max_nodes } = params as {
          ...
          try {
            ...
            return { content: [{ type: "text" as const, text: formatCortexToolResult("Cortex graph", result) }] };
          } catch (err) {
            return { content: [{ type: "text" as const, text: `Cortex graph query failed: ${String(err)}` }] };
          }
        },
      },
      { name: "cortex_graph_query" },
    ...
    api.registerTool(
      {
        ...
        parameters: Type.Object({
          ...
          source_scope: Type.Optional(Type.Union([
            Type.Literal("internal"),
            Type.Literal("customer_accounts"),
          ], { description: "Optional Company Brain account source scope filter" })),
          account_key: Type.Optional(Type.String({ description: "Optional stable business account key, for example company:electricsheep-internal" })),
        ...
        async execute(_toolCallId: string, params: unknown): Promise<any> {
          const { search, source_scope, account_key, workspace_id, limit, offset } = params as {
            search?: string;
            source_scope?: CompanyBrainSourceScope;
            account_key?: string;
            workspace_id?: string;
            limit?: number;
            offset?: number;
